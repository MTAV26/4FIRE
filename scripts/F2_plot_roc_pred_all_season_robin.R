...
load(paste0(dir_results, "/roc_area_pred_DJF_",method,"_MSWEP.RData"))
roca_djf=best_roc_pred
load(paste0(dir_results, "/sig_DJF_",method,"_MSWEP.RData")) # mantengo el observado porque la predicción es < 0.01
# S5;best_roc[best_sig >= 0.01] = NA # no lo necesito porque se guardó solo lo significativo
sig_djf=best_sig
rm(best_roc_pred, best_sig)
final_mask_djf <- ifelse((!is.na(mask_djf) & !is.na(sig_djf)), 1, NA)
load(paste0(dir_results, "/roc_area_pred_MAM_",method,"_MSWEP.RData"))
load(paste0(dir_results, "/sig_MAM_",method,"_MSWEP.RData"))
roca_mam=best_roc_pred
sig_mam=best_sig
rm(best_roc_pred, best_sig)
final_mask_mam <- ifelse((!is.na(mask_mam) & !is.na(sig_mam)), 1, NA)

load(paste0(dir_results, "/roc_area_pred_JJA_",method,"_MSWEP.RData"))
load(paste0(dir_results, "/sig_JJA_",method,"_MSWEP.RData"))
roca_jja=best_roc_pred
sig_jja=best_sig
rm(best_roc_pred, best_sig)
final_mask_jja <- ifelse((!is.na(mask_jja) & !is.na(sig_jja)), 1, NA)

load(paste0(dir_results, "/roc_area_pred_SON_",method,"_MSWEP.RData"))
load(paste0(dir_results, "/sig_SON_",method,"_MSWEP.RData"))
roca_son=best_roc_pred
sig_son=best_sig
rm(best_roc_pred, best_sig)
...
dev.off()
pdf(paste(dir_out,"Figure3_",method,".pdf"), width=9, height=6.25)
