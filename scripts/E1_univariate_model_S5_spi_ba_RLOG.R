...
#   # load(paste0(dir_model, "sig_", version, "_", dataset, ".RData"))
#   #   mask_aux[,,idata]=best_sig/best_sig
#   load(file.path(dir_model, paste0("roc_area_", version, "_", dataset, ".RData")))
#   mask_aux[,,idata]=best_roc/best_roc
# }
...
#    }
# }
# image.plot(num_model<9)
# image.plot(mask_model)

# Asumiendo que la longitud de 'datasets' es 11 LA MITAD
# threshold = 9  # Calcula la mitad y redondea hacia arriba
# mask_model= array(NA, dim = c(ni, nj))  # Asegúrate de que ni y nj son correctos y representan las dimensiones de tu grilla.
# 
# for (i in 1:ni) {
#   for (j in 1:nj) {
#     num_non_na = length(which(!is.na(mask_aux[i, j, ])))
#     if (num_non_na >= threshold) {
#       mask_model[i, j] = 1
#     } else {
#       mask_model[i, j] = NA  # Considera cambiar NA por 0 si quieres evitar problemas al graficar
#     }
#   }
# }
...
#   print(dataset)
#   
  load(file.path(paste0(dir_oss, "SPI3_", dataset, "_1993_2020.RData" )))
  load(file.path(paste0(dir_oss, "SPI6_", dataset, "_1993_2020.RData" )))
  load(file.path(paste0(dir_oss, "SPI12_", dataset, "_1993_2020.RData")))
  ...
  if(season == "DJF"){
    ...
    spipred[, ,, , 1, 1] = aux[, , seq(first_month - 1, (dim(aux)[3]-12), 12),]
    ...
    spipred[, , , , 1, 2] = aux[, , seq(first_month - 1, (dim(aux)[3]-12), 12),]
    ...
    spipred[, ,, , 1, 3] = aux[, , seq(first_month - 1, (dim(aux)[3]-12), 12),]
    
   } else {
      
    ##############################################################################
    load(paste0(dir_s5, paste0("spi3SEAS5_",START,"_",M1,"_",dataset,".RData")))
    spi3pred4[is.infinite(spi3pred4)] = NA
    dim(spi3pred4)
    ...
    #   for (j in 62) {
      if (!is.na(best_m_spi_a_fin[i, j]) & !is.na(mask_model[i, j])) {
        ...
        for (iy in 1:length(years)) {
          ...
          pre_obs[iy] = fit$coefficients[1] + fit$coefficients[2] * spi_a_test
        }  
  ...
  # best_sig_obs = p.adjust(sig_obs, method = "fdr")
  # best_roc_obs[best_sig_obs > 0.01] = NA # pasamos el filtro FDR al modelo de las 11 observacione
  
  # save(best_roc_obs, file = paste0(dir_out, "roc_obs_", version, "_", dataset, ".RData"))
  # save(best_sig_obs, file = paste0(dir_out, "sig_obs_", version, "_", dataset, ".RData"))
...
# save(predBAobs,file = paste0(dir_out, "preBAobs_drop_", version, ".RData"))
# save(predBA,file = paste0(dir_out, "preBApre_4fire_", version, ".RData"))


# load(paste0(dir_out,  "roc_obs_", version, "_CHIRPS.RData"))
# image(best_roc_obs)
# 
# # 
# # # ################################################################################
# # # # DROP and 4FIRE
# # # ################################################################################
# # load(paste0(dir_out, "preBAobs_drop_", version, ".RData"))
# # load(paste0(dir_out, "preBApre_4fire_", version, ".RData"))
# # 
...
# # 
# # dim(predBAobs1)
# # summary(as.vector(predBAobs[,,,11]))
# # 
...
# # image(roca_drop)
# # summary(as.vector(roca_drop))
# roca_drop[sig_drop > 0.01] = NA # pasamos el filtro FDR al ensemble de DROP
# roca_4fire[sig_4fire > 0.01] = NA # pasasmos el filtro FDR al ensemble de 4FIRE
# # image.plot(lon, lat, roca_drop)
# 
# save(roca_drop, file = paste0(dir_out, "roc_drop_", version, ".RData"))
# save(roca_4fire, file = paste0(dir_out, "roc_4fire_", version, ".RData"))
# 
# # 
# # image.plot(roca_drop, col = palette, main = paste0("AUC mean observed ", dataset , ": ",round(mean(roca_drop, na.rm = TRUE),2)))
# # image.plot(roca_4fire, col = palette, main = paste0("AUC mean prediction ", dataset , ": ", round(mean(roca_4fire, na.rm = TRUE),2)))
# # 
# # 
# # summary(as.vector(roca_drop))
# # summary(as.vector(roca_4fire))
# # 
