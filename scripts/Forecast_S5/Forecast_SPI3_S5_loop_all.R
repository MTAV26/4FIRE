...
if (where == 'mac') {
  ...
  dir_s5='C:/Users/Usuario/Dropbox/4FIRE/data/seas5_ajd/'
...
for (istart_date in 1:length(start_dates)) {
  ...
  for (idata in 1:length(datasets)) {
    ...
    ## keep only 2000-2020 period en total 21 años
    prec = obs[, , 229:480]
    ...
    ## spei forecast
    spi3pred4 = array(data = NA, dim = c(dim(prec)[1], dim(prec)[2], dim(prec)[3], num_ens))
    spi3pred3 = array(data = NA, dim = c(dim(prec)[1], dim(prec)[2], dim(prec)[3], num_ens))
    spi3pred2 = array(data = NA, dim = c(dim(prec)[1], dim(prec)[2], dim(prec)[3], num_ens))
    spi3pred1 = array(data = NA, dim = c(dim(prec)[1], dim(prec)[2], dim(prec)[3], num_ens))
    ...
    for (ianni in 1:length(anni)) {
      ...
      for (ires in 1:num_ens) {
        ...
        for (i in 1:dim(prec)[1]) {
          ...
          for (j in 1:dim(prec)[2]) {
            if (!is.na(inout[i, j])) {
              ...
              dum4 <- spi(PRE_FOR4[i, j,], 3, na.rm = TRUE)
              dum3 <- spi(PRE_FOR3[i, j,], 3, na.rm = TRUE)
              dum2 <- spi(PRE_FOR2[i, j,], 3, na.rm = TRUE)
              dum1 <- spi(PRE_FOR1[i, j,], 3, na.rm = TRUE)
        ...
        if (target_season == 'MJJA') {
          ...
          spi3pred4[, , mese_start + 7, ires] = spitmp4[, , mese_start + 7]
          spi3pred3[, , mese_start + 6, ires] = spitmp3[, , mese_start + 6]
          spi3pred2[, , mese_start + 5, ires] = spitmp2[, , mese_start + 5]
          spi3pred1[, , mese_start + 4, ires] = spitmp1[, , mese_start + 4]
          
        }else if (target_season == 'ASON') {
          
          spi3pred4[, , mese_start + 10, ires] = spitmp4[, , mese_start + 10]
          spi3pred3[, , mese_start + 9, ires] = spitmp3[, , mese_start + 9]
          spi3pred2[, , mese_start + 8, ires] = spitmp2[, , mese_start + 8]
          spi3pred1[, , mese_start + 7, ires] = spitmp1[, , mese_start + 7]
          
        } else if (target_season == 'NDJF') {
          
          spi3pred4[, , mese_start + 1, ires] = spitmp4[, , mese_start + 1]
          spi3pred3[, , mese_start    , ires] = spitmp3[, , mese_start    ]
          spi3pred2[, , mese_start - 1, ires] = spitmp2[, , mese_start - 1]
          spi3pred1[, , mese_start - 2, ires] = spitmp1[, , mese_start - 2]
          
        } else if (target_season == 'FMAM') {
          
          spi3pred4[, , mese_start + 4, ires] = spitmp4[, , mese_start + 4]
          spi3pred3[, , mese_start + 3, ires] = spitmp3[, , mese_start + 3]
          spi3pred2[, , mese_start + 2, ires] = spitmp2[, , mese_start + 2]
          spi3pred1[, , mese_start + 1, ires] = spitmp1[, , mese_start + 1]
    ...
    if (target_season == 'MJJA') {
      ...
      save(spi3pred4, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_AUG_", dataset, ".RData", sep = "") ))
      save(spi3pred3, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_JUL_", dataset, ".RData", sep = "") ))
      save(spi3pred2, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_JUN_", dataset, ".RData", sep = "") ))
      save(spi3pred1, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_MAY_", dataset, ".RData", sep = "") ))
      
    }else if (target_season == 'ASON') {
      
      save(spi3pred4, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_NOV_", dataset, ".RData", sep = "") ))
      save(spi3pred3, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_OCT_", dataset, ".RData", sep = "") ))
      save(spi3pred2, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_SEP_", dataset, ".RData", sep = "") ))
      save(spi3pred1, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_AUG_", dataset, ".RData", sep = "") ))
      
    } else if (target_season == 'NDJF') {
      
      save(spi3pred4, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_FEB_", dataset, ".RData", sep = "") ))
      save(spi3pred3, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_JAN_", dataset, ".RData", sep = "") ))
      save(spi3pred2, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_DEC_", dataset, ".RData", sep = "") ))
      save(spi3pred1, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_NOV_", dataset, ".RData", sep = "") ))
      
    } else if (target_season == 'FMAM') {
      
      save(spi3pred4, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_MAY_", dataset, ".RData", sep = "") ))
      save(spi3pred3, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_APR_", dataset, ".RData", sep = "") ))
      save(spi3pred2, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_MAR_", dataset, ".RData", sep = "") ))
      save(spi3pred1, file = file.path(dir_out, paste("spi3SEAS5_", sprintf("%02d", start_date),"_FEB_", dataset, ".RData", sep = "") ))
